...
1. Create an empty file with a text editor and save it as icons.css in your template root directory.
...
Now you need to tell the template which images to load. First make sure that the images you want to use are located in the template directories. For example if you’re using the "Protostar" template then you could place the images in:
...
Download the font that you created and unzip the file. Within this file you will find a couple of files and directories. Be sure to copy all the fonts from the "fonts" directory and add these files to your template. If you are using the “Protostar” template you could (for example) add the font files to:
